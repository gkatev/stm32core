...
#define MAGICWORD_LOAD_DFU 0xF103DFB1
#define MAGICWORD_LOAD_APP 0xECBC6FA3
...
static void write_magic_word(uint32_t magic_word);
static uint32_t get_magic_word(void);
static void kill_usb(void);
static bool try_jump_to_application(void);
...
static void calculate_usb_serialnum(void) {
...
static void write_magic_word(uint32_t magic_word) {
	...
	BKP_DR1 = magic_word >> 16;
	BKP_DR2 = magic_word & 0xFFFF;
...
static uint32_t get_magic_word(void) {
	...
	word = ((uint32_t) BKP_DR1 << 16) | BKP_DR2;
...
static bool try_jump_to_application(void) {
	...
	// Will never actually return from this point
	return true;
}
...
int main(void)
{
	...
	uint32_t magic_word = get_magic_word();
	
	bool has_app_word = (magic_word == MAGICWORD_LOAD_APP);
	bool has_dfu_word = (magic_word == MAGICWORD_LOAD_DFU);
	bool has_dfu_pin = gpio_get(GPIOB, GPIO2);
	
	if(has_app_word || !(has_dfu_word || has_dfu_pin))
		try_jump_to_application();
